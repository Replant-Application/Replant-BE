...
public interface PostRepository extends JpaRepository<Post, Long> {
       ...
       @Query(value = "SELECT p FROM Post p " +
                     ...
                     "AND (p.delFlag = false OR p.delFlag IS NULL)", countQuery = "SELECT COUNT(p) FROM Post p " +
                                   "WHERE (p.postType = 'GENERAL' OR p.postType = 'VERIFICATION') " +
       ...
       Page<Post> findByUserIdAndPostType(@Param("userId") Long userId, @Param("postType") PostType postType,
                     Pageable pageable);
